% =========================================================================
% Description: 在同一 SR 条件下，对比 PGPSO 和标准 PSO 在盲优化指标下的性能
%              - PSO-RSCM：标准 PSO 算法优化 RSCM 适应度
...
% 相同的评价指标
evaluator = @(x) RSCMEvaluator(x, clean_sig, noise_seq, fs, bpg_opts);
...
[fit_bpg, theta_bpg, hist_bpg] = PGPSO( ...
    theta_min, theta_max, bpg_opts, evaluator);

fprintf('标准 PSO 单次最优盲适应度 J = %.4f\n', fit_std);
...
xlabel('迭代次数'); ylabel('全局最优盲适应度 J');
legend('PSO','PGPSO','Location','best');
title('RSCM 收敛曲线');
